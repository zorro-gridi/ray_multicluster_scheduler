...
"""
增强的并发Actor测试用例
测试多个Actor并发执行和退出机制，重点验证多集群并发支持和连接管理优化
"""
...
def concurrent_actor_test():
    ...
    try:
        ...
        actor_configs = [
            ...
            {"name": "actor_4", "preferred_cluster": "mac"},
            {"name": "actor_5", "preferred_cluster": "centos"}
        ]
        ...
        # 分批获取结果，增加超时处理
        for i, task_info in enumerate(task_refs):
            try:
                # 为每个任务设置超时时间
                result = ray.get(task_info['ref'], timeout=10.0)
        ...
        logger.info("✅ 增强的并发Actor测试完成")
        return len(results) > 0, task_lifecycle_manager
        
    except Exception as e:
        logger.error(f"❌ 增强的并发Actor测试出错: {e}")
        import traceback
...
if __name__ == "__main__":
    ...
    try:
        ...
        if success:
            logger.info("🎉 增强的并发Actor测试通过")
            sys.exit(0)
        else:
            logger.error("💥 增强的并发Actor测试失败")
            sys.exit(1)
